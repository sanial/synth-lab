...
/**
 * Analysis payload rendered by the Research Agent panel.
 */
interface AnalysisResult {
  /** Narrative analysis text generated by Gemini. */
  analysis: string;
  /** Supporting Mermaid diagram snippets for detailed views. */
  subDiagrams: string[];
  /** Optional base64-encoded MP3 narration. */
  audioData?: string;
}

/**
 * Props for the AnalysisAgent component.
 */
interface AnalysisAgentProps {
  /** Papers selected in the parent view for synthesis. */
  papers: ArxivPaper[];
  /** Primary diagram context from the synthesis workflow. */
  mainDiagram: string;
  /** Optional precomputed analysis payload to hydrate the panel immediately. */
  preloadedResult?: AnalysisResult | null;
}

/**
 * Research Agent panel that generates deep synthesis text, optional narration,
 * and renders supporting sub-diagrams for selected papers.
 *
 * @param props Component props.
 * @param props.papers Selected paper inputs.
 * @param props.mainDiagram Main synthesis diagram passed by parent context.
 * @param props.preloadedResult Optional previously generated analysis payload.
 * @returns Rendered Research Agent experience.
 */
export const AnalysisAgent: React.FC<AnalysisAgentProps> = ({ papers, mainDiagram, preloadedResult }) => {
  const [analyzing, setAnalyzing] = useState(false);
  const [result, setResult] = useState<AnalysisResult | null>(preloadedResult || null);
  ...
  /**
   * Triggers synthesis for selected papers and asynchronously requests
   * narration audio for the resulting analysis text.
   *
   * @returns Promise that resolves when analysis fetch completes.
   */
  ...
  /**
   * Converts a base64 string into a Blob for browser audio playback.
   *
   * @param base64 Base64-encoded binary content.
   * @param type MIME type for the resulting blob.
   * @returns Blob instance suitable for object URL creation.
   */
  ...
  /**
   * Toggles narration audio playback for the current analysis.
   *
   * @returns Void.
   */
  const togglePlay = () => {
